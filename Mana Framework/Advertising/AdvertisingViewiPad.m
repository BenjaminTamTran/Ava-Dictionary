...
#import "AdvertisingViewiPad.h"

#define POPLISTVIEW_SCREENINSET 100.
#define POPLISTVIEW_HEADER_HEIGHT 120.
#define RADIUS 10.

@interface AdvertisingViewiPad (private)
- (void)fadeIn;
- (void)fadeOut;
@end

@implementation AdvertisingViewiPad {
    NSString *_title;
    NSString *_appStore;
...
- (id)initWithTitle:(NSString *)aTitle options:(NSArray *)aOptions andDescription:(NSString*)desc {
    ...
    if (self = [super initWithFrame:rect]) {
        ...
        UILabel *description = [[UILabel alloc] initWithFrame:CGRectMake(_imageScroll.frame.origin.x + 80.0, _imageScroll.frame.origin.y +  _imageScroll.frame.size.height, 425.0, 90.0)];
        ...
        button.frame = CGRectMake(description.frame.origin.x, description.frame.origin.y +  description.frame.size.height + 10.0f, 425.0, 50.0);
        ...
        [button2.titleLabel setFont:[UIFont systemFontOfSize:25.]];
        button2.frame = CGRectMake(button.frame.origin.x, button.frame.origin.y +  65.0f, 425.0, 50.0);
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if ([_delegate respondsToSelector:@selector(AdvertisingViewiPadDidCancel)])
        [_delegate AdvertisingViewiPadDidCancel];
...
- (void)drawRect:(CGRect)rect {
    ...
    CGRect titleRect = CGRectMake(POPLISTVIEW_SCREENINSET + 30, POPLISTVIEW_SCREENINSET + 30 + 5,
                                  rect.size.width -  2 * (POPLISTVIEW_SCREENINSET + 10), 80);
    ...
    [_title drawInRect:titleRect withFont:[UIFont systemFontOfSize:25.]];
